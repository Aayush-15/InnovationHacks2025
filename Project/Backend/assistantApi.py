...
def authorize_both():
    ...
    if gmail_authed and calendar_authed:
        return HTMLResponse("<h3>✅ Already Authorized for Gmail & Calendar. Use /email or /calendar</h3>")
    
    if not gmail_authed:
        authenticate_gmail()
    if not calendar_authed:
        authenticate_calendar()

    return HTMLResponse("<h3>✅ Authorization Complete. Now visit <a href='/email'>/email</a> or <a href='/calendar'>/calendar</a></h3>")
